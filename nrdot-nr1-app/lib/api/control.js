...
export class OptimizationControlAPI {
  ...
  static async setProfile(accountId, profile, metadata = {}) {
    const mutation = `
      mutation SetOptimizationProfile($accountId: Int!, $document: NerdStorageDocument!) {
    ...
    if (error || !data?.nerdStorageWriteDocument?.success) {
  ...
  static async applyToHosts(accountId, hostFilters, profile) {
    ...
    for (const chunk of chunks) {
      const mutation = `
        mutation BulkSetProfile($accountId: Int!, $document: NerdStorageDocument!) {
          nerdStorageWriteDocument(
            accountId: $accountId,
            collection: "${CONTROL_COLLECTION}",
            documentId: "bulk-${Date.now()}",
            document: $document
          ) {
            success
            errors
      ...
      results.push({
        chunk,
        success: data?.nerdStorageWriteDocument?.success,
        error
      });
    }
