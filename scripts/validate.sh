#!/bin/bash
# Unified NRDOT Validation Script
# Combines functionality from all validation scripts into a modular design
# Supports: Docker, Native, OTEL Config, Complete Setup validation
...
Commands:
    ...
    otel            Validate OpenTelemetry configuration
    complete        Validate complete setup (all components)
...
Examples:
    ...
    $0 otel                     # Validate OpenTelemetry configuration
...
parse_args() {
    ...
    # Validate command
    case $command in
        deployment|services|metrics|queries|integration|otel|complete|all|quick) 
...
validate_otel_config() {
    ...
    # Configuration paths
    local CONFIG_DIR="$PROJECT_ROOT/distributions/nrdot-plus/config"
    local CONFIG_FILE="$CONFIG_DIR/config.yaml"
    local TEST_LICENSE_KEY="test-key-12345678901234567890123456789012345678"
    
    # Check if config file exists
    run_test "OTEL Config File Exists" \
        "[ -f \"$CONFIG_FILE\" ]" \
        ""
    
    # Check if config file is valid YAML
    run_test "OTEL Config Syntax" \
        "command -v yamllint >/dev/null 2>&1 && yamllint -d relaxed \"$CONFIG_FILE\" >/dev/null 2>&1 || (command -v yq >/dev/null 2>&1 && yq eval . \"$CONFIG_FILE\" >/dev/null 2>&1)" \
        "command -v yamllint >/dev/null 2>&1 || command -v yq >/dev/null 2>&1 || (command -v brew >/dev/null 2>&1 && brew install yq)"
    
    # Check if config has the required sections
    run_test "OTEL Config Structure" \
        "grep -q 'receivers:' \"$CONFIG_FILE\" && grep -q 'processors:' \"$CONFIG_FILE\" && grep -q 'exporters:' \"$CONFIG_FILE\" && grep -q 'service:' \"$CONFIG_FILE\"" \
        ""
    
    # Check if exporters section has New Relic configuration
    run_test "OTEL New Relic Exporter" \
        "grep -A10 'exporters:' \"$CONFIG_FILE\" | grep -q 'otlp\\|newrelic'" \
        ""
    
    # Check if environment variable substitution is properly configured
    run_test "OTEL Environment Variables" \
        "grep -q '\\${' \"$CONFIG_FILE\"" \
        ""
    
    log_info "OpenTelemetry configuration validation complete"
}
...
validate_complete_setup() {
    ...
    log_info "Complete setup validation finished"
}
...
2. Run \`./scripts/validate.sh metrics --verbose\` for detailed metrics analysis
3. Check cost reduction KPIs
4. Review control loop operation

---
*Generated by Unified NRDOT Validation Script*
EOF
    fi
...
main() {
    echo "======================================"
    echo " NRDOT v2 Unified Validation Tool"
...
# Handle errors
trap 'log_error "Validation failed! Check the error above."; exit 1' ERR
